        ...
        <div className='px-3 mb-60'>
            ...
            <div>
                ...
                selected.map(select => <div key={select.players_id} className='flex justify-between w-full items-center p-2 border-2 border-gray-400 rounded-xl mb-5'>
                    ...
                    <div><img className='w-[60px] md:w-[100px] h-[80px] md:h-[100px] border-2 rounded-xl' src={select.image} alt="" /></div>
                    <div className='space-y-2'>
                        <h1 className='text-lg md:text-2xl font-bold'>{select.name}</h1>
                        <p className='text-gray-500'>{select.role}</p>
                        <p className='text-gray-500'>Price: {select.biddingPrice}</p>
                    </div>
                    </div>
                    <div>
                        <button onClick={() =>handleDelete(select.playerId, select.name)}>
